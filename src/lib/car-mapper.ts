...
import type { ApiCar, FuelType, ListingType, TransmissionType } from "./api-types";
...
export function mapApiCarToView(car: ApiCar): CarView {
  const orderedImages = [...(car.images ?? [])]
    ...
    .map((image) => resolveAssetUrl(image.url ?? image.imageUrl))
    .filter(Boolean);
...
export function mapApiCarsToView(cars: ApiCar[]) {
  return cars.map(mapApiCarToView);
}
